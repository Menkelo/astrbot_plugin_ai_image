...
- ✅ **比例与分辨率控制**：支持 11 种宽高比（如 1:1、16:9、竖屏等），支持 1K/2K/4K 分辨率。OpenAI gpt-image 系列按官方 SIZE_MAPPING 发送精确分辨率（如 2K 16:9=`2048x1152`、4K 16:9=`3840x2160`），服务端拒绝 size 时自动去掉重试并本地按目标长边放大兜底。
- ✅ **智能比例识别**：图生图时自动推断参考图比例，无需手动指定。
- ✅ **全局预设系统**：使用 AstrBot 全局预设，支持快速调用，可动态管理（增/删）。
- ✅ **多 API Key 轮询**（Vertex）：Vertex 渠道支持配置多组凭证，失败或限流时自动切换。
- ✅ **失败重试**：生成失败时自动重试（次数与间隔可配置），提高成功率。
...
| `max_retries` | int | `3` | 失败重试次数（含首次尝试），最小为 1 |
| `retry_interval` | int | `1` | 每次重试之间的等待秒数 |
...
### Q: 生成失败怎么办？
A: 插件内置 3 次自动重试，失败后会显示分类后的错误原因及脱敏详情（如「模型不存在」「API Key 无效」「配额或余额不足」等，URL/Key/Token 会被隐藏），完整原始错误会记录到插件日志。检查 API Key 是否有效、配额是否充足、模型名是否正确。
