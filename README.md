...
- ✅ **保留完整画面**：未指定比例时，以第一张参考图应用 EXIF 方向后的实际比例为本地目标，其他参考图只作为辅助。输入不再先补成正方形，模型返回比例不符时等比适配并补边，不猜测边缘是否可以裁掉；此规则也适用于显式指定比例和备用接口。
...
### Q: 为什么新图片可能出现补边？
A: 当模型返回的比例与目标不一致时，插件优先保留模型已返回的完整画面，通过补边调整比例，不会自动裁掉边缘内容。原图有实际透明区域时补透明边，否则补白边。正常大小的图片保留原像素，只有超过画布上限时才等比缩小。分辨率档位控制请求尺寸与最小输出长边，补边可能让最终画布变大；“跟随参考图比例”不代表强制输出与参考图完全相同的像素数。详见 [比例处理说明](docs/IMAGE_RATIO.md)。
