...
使用支持插件 Pages 的 AstrBot（已按 v4.28.0 验证），更新插件后重载，在 **插件 → AI 绘图聚合 → 图片管理** 打开「映像」图库。

新生成的图片默认自动保存，可搜索提示词、收藏、整理相册和标签、批量下载，误删后可从回收站恢复。也可以直接拖入本地 PNG/JPEG/WebP/GIF 图片。

图库配置位于 `gallery_config`，数据保存在 `data/plugin_data/astrbot_plugin_ai_image/gallery/`。旧版的临时图片不会自动补入历史，可手动导入。完整用法见 [图片管理说明](docs/GALLERY.md)。

## 方式一：使用系统提供商（推荐）

1. 在 AstrBot 配置页面添加提供商
2. 在插件配置的主提供商（`provider_1`）中选择已添加的提供商 ID
3. 可选：在备用提供商（`provider_backup`）中选择另一个提供商 ID，主接口失败一次后立即请求备用接口
4. 配置完成，发送 `/生图 测试图片` 试试
...
> 比例由指令内嵌指定（如 `/生图 风景 16:9`）；未指定时，图生图按参考图比例推断。
